<template>
  <div>
    <Table
      title="Atendentes de médico cadastrados"
      font-size="1.5rem"
      :items="$all"
      :headers="headers"
...
const atendentStore = useUserAtendentStore();
const { formatTelephoneNumber } = useUtils();
const $all = computed(() => atendentStore.$all);
...
const handleSearch = useDebounceFn(
  async (search: string, isLoading: boolean = true) => {
    ...
    try {
      await atendentStore.index(search);
    } finally {
      loading.value = false;
    }
  },
...
const handleDeleteItem = async () => {
  ...
  loading.value = true;
  try {
    await atendentStore.destroy(selected.value?.publicId!);
